...
http.createServer((req, res) => {
  ...
  switch (req.url){
    case "/hola":
      ...
      break;
      default:
        res.write("mi primer sevidor con http");
  }
  res.end();

}).listen(3001)

console.log("Se esta escuchando desde el puerto 3001");
*/
...
app.get('/:name', (req, res) => {
  ...
  const booksFiltered = books.filter(obj => Object.values(obj).some(val => val.toString().includes(name)));

  if (booksFiltered){
    res.send(booksFiltered);
  }
  else {
    res.send("No existen ninguna coincidencia para: " +name);
  }
})
*/
